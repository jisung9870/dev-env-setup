...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/workbench-sessionizer.XXXXXX")"
trap 'rm -rf "$tmp_root"' EXIT

mkdir -p \
  "$tmp_root/parent projects/alpha project" \
  "$tmp_root/parent projects/beta" \
  "$tmp_root/parent projects/.hidden" \
  "$tmp_root/direct project"

sed "s|@ROOT@|$tmp_root|g" "$fixture_dir/dirs" >"$tmp_root/dirs"
sed "s|@ROOT@|$tmp_root|g" "$fixture_dir/expected-projects" | sort >"$tmp_root/expected"
